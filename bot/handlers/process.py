...
async def process_files(message: types.Message, state: FSMContext) -> None:
    ...
    if not files:
        ...
        await maybe_await(message.answer("⚠️ Нет файлов для обработки. Загрузите их сначала (отправьте JSON файлы)."))
        return

    status_msg = await maybe_await(message.answer("⏳ Начинаю обработку файлов..."))

    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)

            file_paths = await maybe_await(download_files(
                message.bot,
                files,
                temp_path,
        ...
        if status_msg is not None and hasattr(status_msg, "delete"):
            await maybe_await(status_msg.delete())
        ...
        await maybe_await(message.answer(f"❌ Произошла ошибка при обработке: {e}"))
